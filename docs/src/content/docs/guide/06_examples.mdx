...
<div class="[&_strong]:font-semibold">
  ...
  <Aside type="tip">
    ...
    ➡️ This means, you can **_save_ these `.svg` files** to your local disk, and **_open_ with Excalidraw!**
  </Aside>
  ...
  <AstroFancybox image="/assets/examples/kstd-example_ecommerce-product-view_v0.5.0.svg"
                 width={3120}
                 height={1421}
                 classAppend="!mt-8 !mb-12"
  ...
  Four Kafka topics provide the source data to be aggregated. A 'Notification Service' REST API triggers user notifications.
  ...
  <AstroFancybox image="/assets/examples/kstd-example_user-booking-flight-status-notifications_v0.3.5.svg"
                 width={744}
                 height={1930}
  ...
  #### Events
  - **Flight**: all details about scheduled flights, including flight category, flight number and departure date, as well as departure and arrival airports and times.
  - **AirportInfoI18n**: transports information about airports globally in different languages. The schema includes mandatory information in English, with the option to include translations in other languages.
  - **UserFlightBooking**: holds information about a traveler’s booked flight, including reservation details, contact information, and preferred language for notifications.
  - **FlightStatusUpdate**: focuses on the flight as a whole rather than just the departure airport. It includes information about the current status of the flight, any changes to the flight schedule, and delays.
  - **UserFlightBookingNotification**: joins and combines information from _UserFlightBooking_, _Flight_, _FlightStatusUpdate_, and _AirportInfoI18n_ into an enriched output event, which is emitted on all flight status updates
