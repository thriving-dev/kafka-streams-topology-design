...
<div class="images-zoomable">
    ![KSTD 'WordCount Demo' tutorial step 3: Processing Steps, Output Topic](../../../assets/tutorial/kstd-tutorial-wordcount_v0.1.0.png)
</div>
...
<SwiperGallery name="step4" classAppend="!mt-4 py-4 [&>div]:max-w-[600px] flex justify-center items-center">
    <div class="swiper-slide">![KSTD 'WordCount Demo' tutorial step 4: Details Snippet (1)](../../../assets/tutorial/kstd-tutorial-wordcount-step-4a.png)</div>
    <div class="swiper-slide">![KSTD 'WordCount Demo' tutorial step 4: Details Snippet (2)](../../../assets/tutorial/kstd-tutorial-wordcount-step-4b.png)</div>
    <div class="swiper-slide">![KSTD 'WordCount Demo' tutorial step 4: Details Snippet (3)](../../../assets/tutorial/kstd-tutorial-wordcount-step-4c.png)</div>
</SwiperGallery>
...
<div class="images-zoomable">
    ![KSTD 'WordCount Demo' tutorial step 4: Completed Tutorial](../../../assets/tutorial/kstd-tutorial-wordcount_v0.2.0.png)
</div>
...
<Aside>
  **We're now entering *low level grounds* of the Kafka Streams *internal architecture*.**

  For most use cases where the high-level Kafka Streams DSL is used, the design of the application (topology) does not need
  to emphasize details such as sub-topologies, repartitioning, and changelog topics.
</Aside>

In this bonus step, we're adding the following details to our design:
- Named Processing Steps: (above each step) all Processor nodes are named.<br />
  *(Why is this important? -> [KSTD Standard](/guide/kstd-standard/#named-processing-steps))*
- Internal concepts such as *repartition topics* and *state stores*, using [advanced notation indicators](/notation/advanced/)
- [Sub-topologies](/notation/fundamentals/#sub-topology) (dotted line blue boxes) show how Kafka Streams splits up topologies under the hood due to stream repartitioning.

<AstroFancybox image="/assets/examples/kstd-example_wordcount-demo_0.3.0.svg"
               width={2241}
               height={871}
               classAppend="!mt-8 !mb-12"
               caption="Example: WordCount Demo"
               alt="KSTD 'WordCount Demo' tutorial - final result"/>

<Aside type="caution">
  [Sub-topologies](https://docs.confluent.io/platform/current/streams/architecture.html#sub-topologies-also-called-sub-graphs) in particular are determined by the KS framework itself and cannot be changed directly by developers.
  For advanced use-cases it can be important to understand and consider.
  Please note that it requires experience and cross-checking the actual KS runtime to get it right.

  ℹ️ For more details, discussing the correct level of detail of your diagrams, visit [Design Principles > 2. Prioritize Clarity Over Completeness](/guide/design-principles/#2-prioritize-clarity-over-completeness).
</Aside>
