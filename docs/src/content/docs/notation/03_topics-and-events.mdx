---
title: Topics & Events
description: Learn about different topic components, key-value record notation, and SerDes handling in Kafka Streams topologies.
slug: notation/topics-and-events
sidebar:
  order: 3
head:
  - tag: title
    content: Notation > Topics & Events | The standard for designing and visualising Kafka Streams Topologies
---
...
<LibraryComponent name="record-kv" width={110} title="Key-Value Record."
...
<LibraryComponent name="record-kv-typed" width={111} title="Key-Value Record Typed."
                  description="<br>Includes additional information on the data types for key and value. SerDes to be used can be resolved from these types."/>

<LibraryComponent name="record-envelope" width={53} title="Record/Message/Event (Envelope)."
                  description="<br>A simple envelope icon for specific/advanced documentation purposes, e.g. to highlight messages being processed or transmitted."/>

<LibraryComponent name="tombstone" width={44} title="Tombstone."
                  description="<br>Deletions (<i>null</i> values) are called <i>tombstones</i> within the Kafka context."/>
