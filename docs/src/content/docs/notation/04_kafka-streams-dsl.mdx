...
<Aside>
  Some of the visual components cover _multiple operations_ that functionally belong closely together.
  Example: `filter` vs. `filterNot`.

  To avoid bloating the Excalidraw library with _identical_ components that _differ only by label_, such varieties have to be accounted for by the user.

  => to get a `leftJoin`, take the `join` component and _modify+ the label.
  ("double-double" click on the text)
</Aside>
...
### filter; filterNot
<LibraryComponent name="dsl-filter" width={64} description="<p><b>filter</b>s records based on a specified predicate, allowing only records that satisfy the condition to pass through.</p>"
operation="Stateful"/>
...
operation="Stateless, key-changing"/>
...
operation="Potentially stateful and/or key-changing"/>
...
operation="Potentially stateful"/>
...
operation="Stateful (KTable)"
notes="<i>Note: <code>transform</code> & <code>transformValues</code> also exist for KStream, but are deprecated. Use <code>process</code>/<code>processValues</code> instead.</i>"/>


### split +branch
<LibraryComponent name="dsl-split-branch" width={64} description="<p><b>split</b> splits a KStream into multiple branches based on a user-defined predicate, allowing different parts of the stream to be processed separately.</p>"
operation="Stateless"/>
...
### join; leftJoin; outerJoin, (fk *join)
<LibraryComponent name="dsl-join" width={64} description="<p><b>join</b>/<b>leftJoin</b>/<b>outerJoin</b> combines records from two KStreams or a KStream and a KTable, matching records with the same key within a defined time window.</p>"
operation="Stateful"/>

<LibraryComponent name="dsl-join-key-changing" width={84} description="<p><b>fk join</b>/<b>leftJoin</b>/<b>outerJoin</b> <b>(*foreign-key join, key-changing)</b> allows records with non-matching keys to be joined by applying a <i>foreignKeyExtractor</i>.</p>"
operation="Stateful, key-changing"/>
...
operation="Stateless, key-changing (...for groupBy)"/>

### windowedBy
<LibraryComponent name="dsl-windowed-by" width={102} description="<p><b>windowedBy</b> applies a time windowing strategy to a grouped stream, partitioning records into defined intervals for subsequent aggregations.</p>"
operation="Stateful"/>
